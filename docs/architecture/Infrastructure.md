...
**PSP:** Stripe only (not Mollie). Product rules: [docs/domain/14_subscription_entitlements.md](../domain/14_subscription_entitlements.md). Column reference: [docs/architecture/db.md](db.md).

**Implemented in Postgres today** (`20260321000001_super_admin_*`, `20260321000002_institution_admin_*`):

| Table                         | Role                                                                                      | Stripe-relevant columns                                                                                                                                                                                                     |
| ----------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `plan_catalog`                | Commercial plans (`trial` seeded in `2026032100000105_super_admin_05_backfills_seed.sql`) | `code`, `price_amount`, `currency`, `billing_interval`, `seat_cap_default`, `storage_bytes_cap_default` — map Stripe Product/Price to `plan_catalog.code`; store active Price id in `billing_providers.external_price_id`   |
| `institution_subscriptions`   | Per-tenant subscription state                                                             | `billing_status` (`billing_status` enum), `plan_id`, `renewal_at`, `grace_ends_at`, `trial_ends_at`, `current_period_start` / `current_period_end`, `cancel_at_period_end`, `canceled_at`, `seats_cap`, `storage_bytes_cap` |
...
**Stripe docs:** [Checkout](https://docs.stripe.com/payments/checkout) · [Subscriptions](https://docs.stripe.com/billing/subscriptions/overview) · [Webhooks](https://docs.stripe.com/webhooks) · [Customer Portal](https://docs.stripe.com/customer-management/customer-portal)
...
- `caddy` + https://caddyserver.com/docs/?utm_source=perplexity
...
2. **Data Processing Agreement (AVV / DPA)**
   ...
   - Must sign with: Hetzner, Brevo, Stripe (payments), any analytics provider
...
**Stripe (PSP).** Schema und RLS sind migrationsseitig fertig; der **Webhook-Service fehlt noch** im Repo. Stripe läuft über Checkout / Subscriptions / Customer Portal auf demselben Hetzner-Host (Edge Function hinter Kong, z. B. `/functions/v1/stripe-webhook`). Keine extra Server-Kosten — nur API-Traffic.

**Env (self-hosted Supabase):** `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, optional `STRIPE_PUBLISHABLE_KEY` im Frontend. Webhook-Handler mit Service-Role-Client oder dedizierter RPC — nicht PostgREST vom Browser.

**Stripe event → Postgres (target mapping for implementers):**

| Stripe event                              | Write target                                                                                                                                                                                                                                                |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `checkout.session.completed`              | Upsert `billing_providers` (`provider='stripe'`, customer + subscription + price ids from session); set `institution_subscriptions.plan_id` from `plan_catalog` matched by metadata/price; `billing_status='active'`; fill `current_period_*`, `renewal_at` |
| `customer.subscription.updated`           | Update `billing_status` (`active` / `past_due` / `trialing` / `cancelled`), `cancel_at_period_end`, `canceled_at`, period fields                                                                                                                            |
...
5. **Stripe integration** (planned Edge Function — align with migrations above):
   1. Stripe Dashboard: Products/Prices mirror `plan_catalog` (store Price id → `billing_providers.external_price_id`).
   2. Add `supabase/functions/stripe-webhook/index.ts` — verify `stripe-signature`, use **service role** Supabase client.
   3. Add `supabase/functions/create-stripe-checkout-session/index.ts` — `metadata.institution_id` + `metadata.plan_code` for webhook correlation.
   4. Kong route: webhook URL → Edge Functions (no JWT; Stripe signature only).
   5. Super admin can still seed/fix rows via Studio until webhooks go live (`billing_providers` / `institution_subscriptions` super-admin RLS).

   ```ts
   // create-checkout (sketch): institution admin starts paid plan
   const session = await stripe.checkout.sessions.create({
     mode: 'subscription',
     customer: existingStripeCustomerId, // or customer_creation: 'always'
     line_items: [{ price: stripePriceId, quantity: 1 }],
     success_url: `${appUrl}/institution/billing/success`,
     cancel_url: `${appUrl}/institution/billing`,
     metadata: { institution_id: institutionId, plan_code: 'plus' },
   })
   // stripe-webhook: constructEvent → map subscription.status → billing_status enum (note: 'cancelled' not 'canceled')
   ```
