...
COMMENT ON FUNCTION public.create_institution_with_initial_admin(text, uuid, public.membership_status) IS
  'Super admin only: creates institution and institution_admin membership for a non-platform initial admin (p_initial_admin_user_id required when caller is super_admin). Legacy user_institutions only when active; settings, quotas, and trial subscription if plan trial exists.';
