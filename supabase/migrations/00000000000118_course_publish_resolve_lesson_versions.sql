-- =============================================================================
-- COURSE PUBLISH — correct lesson version resolution on (re)publish
--
-- Supersedes the lesson-resolution logic in 20260000000118. That version used
-- `PERFORM app.publish_lesson_version(...)` (which discards the returned id) and
-- then re-selected by `version_major = v_version_no`. For a lesson that had
-- never been published, publish_lesson_version creates version_major = 1, not
-- v_version_no, so the re-select found nothing → 'failed to resolve lesson
-- version'. Fix: capture the returned id directly in both branches.
--
-- Everything else (course/topic snapshotting, the "archive the current delivery
-- before inserting the new one" block that satisfies
-- idx_course_deliveries_current_classroom_course, the public wrapper) is carried
-- forward unchanged from 20260000000118 so this single migration is the complete
-- current truth for the function.
-- =============================================================================
