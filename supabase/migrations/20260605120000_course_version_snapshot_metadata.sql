-- =============================================================================
-- Snapshot course-level metadata on course_versions at publish time.
-- Prevents classroom/published views from reading mutable courses.title/description/theme_id.
-- =============================================================================
...
COMMENT ON COLUMN public.course_versions.title IS
  'Course title copied from courses at publish time; immutable for this version.';
COMMENT ON COLUMN public.course_versions.description IS
  'Course description copied from courses at publish time; immutable for this version.';
COMMENT ON COLUMN public.course_versions.theme_id IS
  'Course theme copied from courses at publish time; immutable for this version.';

UPDATE public.course_versions cv
SET
  title = COALESCE(cv.title, c.title),
  description = COALESCE(cv.description, c.description),
  theme_id = COALESCE(cv.theme_id, c.theme_id)
FROM public.courses c
WHERE c.id = cv.course_id;

ALTER TABLE public.course_versions
  DROP CONSTRAINT IF EXISTS course_versions_theme_id_check;

ALTER TABLE public.course_versions
  DROP CONSTRAINT IF EXISTS chk_course_versions_theme_id;

ALTER TABLE public.course_versions
  ADD CONSTRAINT chk_course_versions_theme_id
  CHECK (
    theme_id IS NULL
    OR theme_id IN (
...
COMMENT ON FUNCTION app.publish_course_to_classrooms IS
  'Publish a teacher-owned course: snapshot course metadata, topics, and lessons; pin topic/lesson versions; create active course_deliveries.';
