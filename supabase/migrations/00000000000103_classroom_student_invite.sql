...
COMMENT ON FUNCTION public.redeem_institution_invite(uuid) IS
  'Authenticated user: accepts non-revoked email invite if profile email matches; creates or activates membership, adds classroom_members when invite.classroom_id is set, flips institution to active for institution_admin invites, and upgrades profiles.role only when the invite is a strict privilege increase.';
