-- Align public.profiles.role with redeemed invite membership (canonical role after redeem).
-- handle_new_user() only allowlists student|teacher from signup metadata; institution_admin is set here.
...
COMMENT ON FUNCTION public.redeem_institution_invite(uuid) IS
  'Authenticated user: accepts email invite if profile email matches; creates or activates membership, user_institutions, sets profiles.role to invite membership_role, flips institution to active when role is institution_admin.';
