...
-- task_templates, and chat conversations.
--
...
-- task_templates
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION audit.log_task_templates_audit()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    v_event_type := 'task_template.created';
  ELSIF TG_OP = 'DELETE' THEN
    v_event_type := 'task_template.deleted';
  ELSIF NEW.deleted_at IS NOT NULL AND OLD.deleted_at IS NULL THEN
    v_event_type := 'task_template.deleted';
  ELSIF NEW.deleted_at IS NULL AND OLD.deleted_at IS NOT NULL THEN
    v_event_type := 'task_template.restored';
  ELSE
    RETURN NEW;
  END IF;
  ...
  PERFORM audit.log_event(
    p_event_type := v_event_type,
    p_subject_type := 'task_template',
    p_subject_id := COALESCE(NEW.id, OLD.id),
    p_institution_id := COALESCE(NEW.institution_id, OLD.institution_id),
    p_payload := '{}'::jsonb,
    p_metadata := jsonb_build_object(
      'visibility_level', 'institution_admin',
      'context', jsonb_build_object('task_template_id', COALESCE(NEW.id, OLD.id))
    )
  );
  RETURN COALESCE(NEW, OLD);
END;
...
DROP TRIGGER IF EXISTS trg_task_templates_audit_row ON public.task_templates;
CREATE TRIGGER trg_task_templates_audit_row
  AFTER INSERT OR UPDATE OR DELETE ON public.task_templates
  FOR EACH ROW EXECUTE FUNCTION audit.log_task_templates_audit();

-- -----------------------------------------------------------------------------
-- conversations (chat lifecycle only — message content is never audited)
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION audit.log_conversations_audit()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    v_event_type := 'conversation.created';
  ELSIF TG_OP = 'DELETE' THEN
    v_event_type := 'conversation.deleted';
  ELSIF NEW.archived_at IS NOT NULL AND OLD.archived_at IS NULL THEN
    v_event_type := 'conversation.archived';
  ELSIF NEW.archived_at IS NULL AND OLD.archived_at IS NOT NULL THEN
    v_event_type := 'conversation.unarchived';
  ELSE
    RETURN NEW;
  END IF;
  ...
  PERFORM audit.log_event(
    ...
    p_metadata := jsonb_build_object(
      'visibility_level', 'institution_admin',
      'context', jsonb_build_object('conversation_id', COALESCE(NEW.id, OLD.id))
    )
  );
  RETURN COALESCE(NEW, OLD);
END;
...
DROP TRIGGER IF EXISTS trg_conversations_audit_row ON public.conversations;
CREATE TRIGGER trg_conversations_audit_row
  AFTER INSERT OR UPDATE OR DELETE ON public.conversations
  FOR EACH ROW EXECUTE FUNCTION audit.log_conversations_audit();
