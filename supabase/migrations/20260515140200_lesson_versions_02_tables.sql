...
-- lesson_progress, and learning_events for version tracking
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS public.lesson_versions (
  ...
  lexical_state JSONB NOT NULL,              -- Reconstructed from lesson_blocks + metadata
...
COMMENT ON COLUMN public.lesson_versions.lexical_state IS
  'Canonical Lexical document JSONB. Serialized from lesson_blocks at publish time.';
...
ALTER TABLE public.lesson_progress
  ADD COLUMN IF NOT EXISTS lesson_version_id UUID
    CONSTRAINT fk_lesson_progress_lesson_versions
    REFERENCES public.lesson_versions(id) ON DELETE RESTRICT;
