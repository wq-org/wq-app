...
-- Requires: 20260000000047_course_delivery_01_types.sql
-- =============================================================================
...
CREATE TABLE public.course_versions (
  ...
  CONSTRAINT chk_course_versions_theme_id CHECK (
    theme_id IS NULL
    OR theme_id IN (
...
COMMENT ON COLUMN public.course_versions.title IS 'Course title copied from courses at publish time; immutable for this version.';
COMMENT ON COLUMN public.course_versions.description IS 'Course description copied from courses at publish time; immutable for this version.';
COMMENT ON COLUMN public.course_versions.theme_id IS 'Course theme copied from courses at publish time; immutable for this version.';
...
CREATE TABLE public.course_version_lessons (
  ...
  CONSTRAINT fk_course_version_lessons_course_versions FOREIGN KEY (course_version_id) REFERENCES public.course_versions (id) ON DELETE CASCADE,
...
COMMENT ON COLUMN public.course_version_lessons.course_version_id IS
  'Denormalized FK to course_versions.id copied from parent course_version_topics.course_version_id. Enables teacher/student RLS policies to join directly to course_versions.';
...
COMMENT ON COLUMN public.course_version_lessons.source_lesson_id IS
  'Original lessons.id at snapshot; learning_events.lesson_id resolves through this source row.';
