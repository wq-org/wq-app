...
const VideoPlayer = React.forwardRef<HTMLVideoElement, VideoPlayerProps>(
  (
    ...
    const toggleMute = React.useCallback(() => {
      ...
      el.muted = !el.muted
      setIsMuted(el.muted)
      setUserUnmuted(!el.muted) // Track if user manually unmuted
    ...
    // Handle focus-based autoplay
    React.useEffect(() => {
      const video = videoRef.current
      if (!video || !autoplay) return

      if (isFocused) {
        // Mute video for autoplay (browser requirement) - but respect user's unmute action
        if (!userUnmuted) {
          video.muted = true
          setIsMuted(true)
        }
        ...
        video.pause()
        // Reset user unmute state when video loses focus
        setUserUnmuted(false)
      }
    }, [isFocused, autoplay, userUnmuted])
    ...
    return (
      ...
      >
        ...
        <video
          ...
          src={src}
          loop={loop}
          autoPlay={autoplay && isFocused}
          muted={autoplay && isFocused && !userUnmuted ? true : isMuted}
          {...props}
        />
        {showControls && (
          <>
            ...
            <div
              className={cn(
                'pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent transition-opacity motion-safe:duration-200',
