...
export function WorkingHoursInput() {
  ...
  const handleToggleDay = (dayId: number) => {
    setLocalWorkingHours((prev) => ({
      ...prev,
      [dayId]:
        prev[dayId].from > 0 || prev[dayId].to > 0 ? { from: 0, to: 0 } : { from: 9, to: 17 },
    }))
  }
  ...
  const handleSave = () => {
    ...
    for (const dayId in updatedWorkingHours) {
      ...
      const isDayActive =
        localWorkingHours[parseInt(dayId)].from > 0 || localWorkingHours[parseInt(dayId)].to > 0
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="space-y-4">
        {DAYS_OF_WEEK.map((day) => {
          const isDayActive =
            localWorkingHours[day.index].from > 0 || localWorkingHours[day.index].to > 0
