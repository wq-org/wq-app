...
export function InstitutionFacultiesCreate() {
  ...
  useEffect(() => {
    ...
    const suggested = buildCohortNameFromTermCode(defaultProgrammeOfferingTermCode)
    setCohortName((current) =>
      isAutoGeneratedCohortName(current) || current === '' ? suggested : current,
    )
  }, [defaultProgrammeOfferingTermCode])

  const CLASS_GROUP_WIZARD_STEP = 4

  // First visit to class-group step: preselect first suggestion (e.g. PREFIX-1A) when the name is still empty.
  useEffect(() => {
    if (wizardStep !== CLASS_GROUP_WIZARD_STEP) return
    const suggestions = generateClassGroupSuggestions(defaultProgrammeOfferingTermCode)
    const first = suggestions[0]
    if (!first) return
    setClassGroupName((prev) => (prev.trim() === '' ? first : prev))
  }, [wizardStep, defaultProgrammeOfferingTermCode])
      ...
      case 3:
        stepContent = (
          <ClassGroupStep
            ...
            cohortAcademicYear={cohortAcademicYear}
            name={classGroupName}
