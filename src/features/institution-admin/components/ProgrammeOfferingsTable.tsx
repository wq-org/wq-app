...
export function ProgrammeOfferingsTable({
  ...
  const [archiveOfferingId, setArchiveOfferingId] = useState<string | null>(null)

  const archiveOffering = useMemo(
    () => offerings.find((o) => o.id === archiveOfferingId),
    [archiveOfferingId, offerings],
  )

  const archiveTermLabel =
    archiveOffering?.term_code?.trim() || t('faculties.pages.programmeOfferings.offering.notSet')

  const handleArchiveConfirmed = () => {
    if (archiveOfferingId) {
      onArchiveOffering?.(archiveOfferingId)
    }
    setArchiveOfferingId(null)
  }
  ...
  return (
    <>
                      ...
                      >
                        <div className="flex flex-col gap-1">
                          ...
                          <Button
                            ...
                            size="sm"
                            className="justify-start"
                            onClick={() => setArchiveOfferingId(offering.id)}
                          >
                            <Archive className="size-4 shrink-0" />
                            {t('faculties.pages.programmeOfferings.timeline.archiveMenuItem')}
                          </Button>
                        </div>
                      </PopoverContent>
                    </Popover>
      ...
      <Dialog
        open={archiveOfferingId !== null}
        onOpenChange={(open) => {
          if (!open) setArchiveOfferingId(null)
        }}
      >
        <DialogContent showCloseButton>
          <DialogHeader>
            <DialogTitle>
              {t('faculties.pages.programmeOfferings.timeline.archiveDialogTitle')}
            </DialogTitle>
            <DialogDescription className="text-pretty">
              <span className="block">
                {t('faculties.pages.programmeOfferings.timeline.archiveDialogDescription')}
              </span>
              {archiveOffering ? (
                <span className="mt-3 block font-medium text-foreground">
                  {t('faculties.pages.programmeOfferings.timeline.archiveDialogOfferingLabel', {
                    term: archiveTermLabel,
                  })}
                </span>
              ) : null}
            </DialogDescription>
          ...
          <DialogFooter className="gap-2 sm:justify-end">
            <Button
              type="button"
              variant="outline"
              onClick={() => setArchiveOfferingId(null)}
            >
              {t('faculties.pages.programmeOfferings.timeline.archiveDialogCancel')}
            </Button>
            <HoldConfirmButton
              variant="orange"
              icon={<Archive className="size-4 shrink-0" />}
              onConfirm={handleArchiveConfirmed}
              disabled={!archiveOffering}
            >
              {t('faculties.pages.programmeOfferings.timeline.holdToArchive')}
            </HoldConfirmButton>
          </DialogFooter>
        </DialogContent>
      </Dialog>
