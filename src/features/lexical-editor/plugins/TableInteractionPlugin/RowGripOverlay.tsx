...
export function RowGripOverlay({
  ...
  const handlePointerDown = (rowIndex: number) => (e: ReactPointerEvent<HTMLButtonElement>) => {
    e.preventDefault()
    e.stopPropagation()
...
function DragInsertionLine({
  ...
  const lineY = rowRect ? (position === 'after' ? rowRect.bottom : rowRect.top) : wrapperRect.bottom
  return (
    <div
