...
export function smartInsertRow(
  ...
  editor.update(() => {
    ...
    const shouldInsertAfter = force
      ? position === 'after'
      : position === 'after' ||
        ($isHeaderRow(referenceRow) && rows[0] === referenceRow && position === 'before')
...
export function smartInsertCol(
  ...
  editor.update(() => {
    ...
    const shouldInsertAfter = force
      ...
      : position === 'after' ||
        (referenceIndex === 0 &&
          $isHeaderColumn(tableNode, referenceIndex) &&
          position === 'before')
...
export function deleteLastEmptyColumn(editor: LexicalEditor, tableKey: NodeKey): boolean {
  ...
  editor.update(() => {
    ...
    if (
      $isHeaderColumn(tableNode, lastColumnIndex) ||
      !$isColumnEmpty(tableNode, lastColumnIndex)
    ) {
      return
    }
...
export function isTableRowHeader(
  editor: LexicalEditor,
  tableKey: NodeKey,
  rowIndex: number,
): boolean {
