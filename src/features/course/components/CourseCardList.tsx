...
export function CourseCardList({
  ...
  return (
    <div
      className={cn(
        'grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-5 animate-in fade-in-0 slide-in-from-bottom-4',
        className,
      )}
    >
      {courses.map((course) => (
        <div
          key={course.id}
          className="h-full animate-in fade-in-0 slide-in-from-bottom-3"
        >
          <CourseCard
            {...course}
            onView={handleView}
